...
| `SUBTRACTION_RESIDUAL_RADIUS_ARCSEC` | `10.0` | No | An uncatalogued subtraction candidate within this radius of a catalogued star of about the same brightness is that star's residual (coma-shifted or imperfectly cancelled PSF), not a transient — suppressed instead of alerted `UNKNOWN`. A transient much fainter than a neighbour this close is suppressed too. `0` disables. |
