...
- [ ] После деплоя проверить `SELECT anomaly_type, COUNT(*) FROM anomalies GROUP BY anomaly_type;`
      и сравнить долю `UNKNOWN`/`MOVING_UNKNOWN` до/после этого фикса
- [ ] Если доля всё ещё велика — значит, доминирующая причина не насыщение,
      а либо ширина `MOVING_CONE_ARCSEC` (120″), либо отсутствие порога по
      магнитуде (Known Issues #1 в CLAUDE.md) — оба варианта тюнинга
      осознанно не тронуты в этом заходе, т.к. требуют данных с прода, а не
      только разбора кода
